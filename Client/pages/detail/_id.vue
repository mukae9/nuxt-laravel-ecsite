    ...
    <div> 
        <div class="item-detail">
            <ul class="item-container">
                ...
                <li ><span class="star">{{$reviewToStar($reviewAverage(item.reviews))}}</span>{{$reviewAverage(item.reviews)}}</li>
                <li>{{item.release}}発売</li>
                <li class="fee">¥{{item.fee.toLocaleString()}}</li>
                <li v-if="item.express"><span class="speed"><span class="round">●</span>速配対応</span>明日までにお届け</li>
            ...
            <div class="container">
                <div class="review-post">
                    <p class="review-post-title">この商品を評価する</p>
                        ...
                        <p>レート</p>
                        <select v-model="review.star" name="star">
                ...
                <div v-if="item.reviews.length">
                    <div v-for="review of item.reviews" :key="review.id" class="review-card">
                        <ul class="review-info">
                            <li><font-awesome-icon icon="user-alt"/>{{review.name}}</li>
                            <li class="star">{{$reviewToStar(review.star)}}</li>
                            <li class="review-day">{{$dateToJaFormat(review.created_at)}} 投稿</li>
                        </ul>
                        <div class="comment">
                            <p>{{review.message}}</p>
                        </div>
...
export default {
    ...
    async asyncData({ app, params}) {
        const url = 'http://localhost/api/detail/'+params.id
