  ...
  <div class="flex flex-col items-center">
    ...
    <form action="#" method="post" class="bg-white rounded-lg p-8 w-full md:w-6/12 lg:w-3/12 shadow-xl" @submit.prevent="submit">
      <div class="mb-4">
        ...
        <input id="email" v-model="form.email" type="text" name="email" class="border-2 border-gray-400 rounded px-3 py-2 w-full">
      </div>
      <div class="mb-6">
        <label for="email" class="block mb-2 text-gray-500 uppercase text-sm font-bold tracking-tight">
          Password
        </label>
        <input id="email" v-model="form.password" type="text" name="email" class="border-2 border-gray-400 rounded px-3 py-2 w-full">
      </div>
      <div>
